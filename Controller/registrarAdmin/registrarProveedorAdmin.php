<?php 
    ...
    if (empty($id_usuario) || empty($tipo_documento_usuario) || empty($nombres_usuario) || empty($apellidos_usuario) || empty($fecha_nacimiento_usuario) || empty($email_usuario) || empty($telefono_usuario)) {
        echo json_encode(['success' => false, 'message' => 'Error, por favor complete los campos para registrar el proveedor']);
    ...
    if ($_FILES['foto_usuario']['error']) {
        ...
        if (in_array($_FILES['foto_usuario']['type'], $PERMITIDOS) && $_FILES['foto_usuario']['size'] <= LIMITE * 1024) {
            ...
            $result = $objConsultas->registrarProveedorAdmin($id_usuario, $tipo_documento_usuario, $nombres_usuario, $apellidos_usuario, $fecha_nacimiento_usuario, $email_usuario, $telefono_usuario, $estado_usuario, $rol_usuario, $clavemd, $foto_usuario);
