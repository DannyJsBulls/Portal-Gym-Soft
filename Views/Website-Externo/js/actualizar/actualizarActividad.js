document.addEventListener('DOMContentLoaded', function () {
    const actualizarActividadForm = document.querySelector("form[name='actualizarActividadForm']");
    const btnActualizarActividad = actualizarActividadForm.querySelector(".btnActualizarActividad");

    btnActualizarActividad.addEventListener('click', async (e) => {
        e.preventDefault();

        const formData = new FormData(actualizarActividadForm);

        try {
            const response = await fetch(actualizarActividadForm.action, {
                method: actualizarActividadForm.method,
                body: formData
            });
