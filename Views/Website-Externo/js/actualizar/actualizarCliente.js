document.addEventListener('DOMContentLoaded', function () {
    const actualizarClienteForm = document.querySelector("form[name='actualizarClienteForm']");
    const btnActualizarCliente = actualizarClienteForm.querySelector(".btnActualizarCliente");

    btnActualizarCliente.addEventListener('click', async (e) => {
        e.preventDefault();

        const formData = new FormData(actualizarClienteForm);

        try {
            const response = await fetch(actualizarClienteForm.action, {
                method: actualizarClienteForm.method,
                body: formData
            });
