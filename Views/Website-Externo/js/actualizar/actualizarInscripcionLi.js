document.addEventListener('DOMContentLoaded', function () {
    const actualizarInscripcionLiForm = document.querySelector("form[name='actualizarInscripcionLiForm']");
    const btnActualizarInscripcionLi = actualizarInscripcionLiForm.querySelector(".btnActualizarInscripcionLi");

    btnActualizarInscripcionLi.addEventListener('click', async (e) => {
        e.preventDefault();

        const formData = new FormData(actualizarInscripcionLiForm);

        try {
            const response = await fetch(actualizarInscripcionLiForm.action, {
                method: actualizarInscripcionLiForm.method,
                body: formData
            });
