document.addEventListener('DOMContentLoaded', function () {
    const actualizarPlanForm = document.querySelector("form[name='actualizarPlanForm']");
    const btnActualizarPlan = actualizarPlanForm.querySelector(".btnActualizarPlan");

    btnActualizarPlan.addEventListener('click', async (e) => {
        e.preventDefault();

        const formData = new FormData(actualizarPlanForm);

        try {
            const response = await fetch(actualizarPlanForm.action, {
                method: actualizarPlanForm.method,
                body: formData
            });
